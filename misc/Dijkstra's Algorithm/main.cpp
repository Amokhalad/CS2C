...
WeightedStringGraph word_graph(const vector<string> &words)
{
    ...
    for (const string &w : words)
    {
        ...
        { // for every character in a word, add an entry with * for example, in cat we would insert, *at, c*t, ca*.
            canonical_word[i] = '*';
...
struct DijkstrasQueueEntry
{
    ...
    bool operator<(const DijkstrasQueueEntry &rhs) const
    { // flips max heap to min heap.
...
vector<WeightedStringGraph::Node> dijkstras(WeightedStringGraph &graph, const WeightedStringGraph::Node &start, const WeightedStringGraph::Node &destination)
{
    ...
    // TODO: make a list of nodes, not just one
    ...
    // If there isn't a path from start to destination, then return an empty path.
    if (prev_node[destination] == destination)
    {
...
{
    vector<string> words;
    for (const string &w : read_words("C:/Users/Mjaya/Desktop/words.txt"))
    {
        if (w.size() == 3)
        {
            words.push_back(w);
    ...
    vector<string> path = dijkstras(graph, "boy", "n");
